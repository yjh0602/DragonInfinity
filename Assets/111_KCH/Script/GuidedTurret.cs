...
public class GuidedTurret : MonoBehaviour
{
    ...
    IEnumerator ShotCoroutine()
    {
        while (true)
        {
            if (targetTransform != null)
            {
                ...
                GuidedBullet newGuidedBullet = newBullet.GetComponent<GuidedBullet>();
                newGuidedBullet.target = targetTransform;

                AudioManager.Instance.PlaySFX("BULLET_FIRE2");
